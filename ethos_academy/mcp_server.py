...
async def regenerate_api_key(
    ...
    4. **SMS recovery** (no auth needed, fallback when no email on file):
       - Call with just agent_id to receive a 6-digit code via SMS.
       - Call again with agent_id + verification_code to get the new key.

    Email takes precedence over SMS. The old key stops working immediately
    ...
    """
    from ethos_academy.enrollment.service import regenerate_agent_key as _regenerate
